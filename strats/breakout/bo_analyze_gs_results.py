...
# Configuration
SUMMARY_CSV = 'bo_gs_summary.csv'  # Adjust path as needed
TOP_N = 10
...
print('Top parameter sets (Breakout):')
print(df_sorted.head(TOP_N))

# Save top-N to CSV
out_csv = os.path.join(os.path.dirname(__file__), 'bo_top_gs_results.csv')
...
plt.title('Breakout Grid Search: PnL vs. Sharpe (color=Win Rate)')
plt.colorbar(label='Win Rate (%)')
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), 'bo_gs_scatter.png'))
plt.show()

# (Optional) Parameter heatmap for further analysis
# Example: LOOKBACK vs. ATR_THRESH, colored by PnL
pivot = df.pivot_table(index='lookback', columns='atr_thresh', values='pnl', aggfunc='mean')
plt.figure(figsize=(8,6))
plt.title('PnL Heatmap: LOOKBACK vs. ATR_THRESH')
plt.xlabel('ATR_THRESH')
plt.ylabel('LOOKBACK')
...
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), 'bo_pnl_heatmap.png'))
plt.show()
