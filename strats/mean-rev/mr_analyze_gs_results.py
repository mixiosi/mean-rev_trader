...
# Configuration
SUMMARY_CSV = 'mr_gs_summary.csv'  # Adjust path as needed
TOP_N = 10
...
# Save top-N to CSV
out_csv = os.path.join(os.path.dirname(__file__), 'mr_top_gs_results.csv')
...
plt.title('Grid Search Results: PnL vs. Sharpe (color=Win Rate)')
plt.colorbar(label='Win Rate (%)')
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), 'mr_gs_scatter.png'))
plt.show()

# (Optional) Parameter heatmap for further analysis
# Example: SMA_LEN vs. ATR_THRESH, colored by PnL
pivot = df.pivot_table(index='sma_len', columns='atr_thresh', values='pnl', aggfunc='mean')
plt.figure(figsize=(8,6))
plt.title('PnL Heatmap: SMA_LEN vs. ATR_THRESH')
...
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), 'mr_pnl_heatmap.png'))
plt.show()
